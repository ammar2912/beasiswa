...
class HistoryPrestasiOrmawa extends E_Controller{

  public function __construct()
  {
    parent::__construct();
    $this->load->model("ModelHistoryPrestasiOrmawa");
  }

  function index()
  {
    $id = $this->session->userdata('id');
    $historyprestasiormawa = $this->ModelHistoryPrestasiOrmawa->get_data($id);
    $data = array(
      'body' => 'HistoryPrestasiOrmawa/list',
      'historyprestasiormawa' => $historyprestasiormawa
     );
  ...
  function tambah_foto(){
    ...
    if(!empty($cek)){
    $data = array(
      'body' => 'HistoryPrestasiOrmawa/tambah_foto',
    ...
    redirect(base_url()."HistoryPrestasiOrmawa");
  }
 }
 ...
 function tambah_foto_proses(){
   ...
   if($_FILES['foto_prestasi']['name']){
     ...
     if($this->upload->do_upload('foto_prestasi')){
       ...
       redirect(base_url()."HistoryPrestasiOrmawa/tambah_foto/".base64_encode($id_prestasi));
     }else{

...
class ApprovalPrestasiOrmawa extends E_Controller{
  ...
  function index()
  {
    $approvalprestasiormawa = $this->ModelApprovePrestasi->get_data_ormawa()->result();
    $data = array(
      'body' => 'ApprovalPrestasiOrmawa/list',
      'approvalprestasiormawa' => $approvalprestasiormawa
     );
  ...
  function approveprestasi()
  {
    $id = base64_decode($this->uri->segment(3));
      ...
      if ($this->db->update("tb_prestasi_mahasiswa",$approvekegiatan)) {
        ...
        redirect(base_url()."ApprovalPrestasiOrmawa");
      }else{
  ...
  function data_foto(){
    ...
    if(!empty($cek)){
    $data = array(
      'body' => 'ApprovalPrestasiOrmawa/data_foto',
      'id_prestasi' => $id,
      'data_foto' => $data_foto,
     );
    $this->load->view('index', $data);

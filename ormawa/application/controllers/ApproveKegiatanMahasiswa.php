...
class ApproveKegiatanMahasiswa extends E_Controller{

  public function __construct()
  {
    parent::__construct();
    $this->load->model("ModelApproveKegiatanMahasiswa");
  }

  function index()
  {
    $approvekegiatan = $this->ModelApproveKegiatanMahasiswa->get_data()->result();
    $data = array(
      'body' => 'ApproveKegiatanMahasiswa/list',
  ...
  function approvekegiatan()
  {
    ...
    $dana_acc = $this->input->post('dana_acc');
      ...
      $this->db->where("id",$id);
      if ($this->db->update("tb_kegiatan_mahasiswa",$approvekegiatan)) {
        $this->session->set_flashdata("notif",$this->Notif->berhasil("Berhasil Approve Kegiatan"));
        redirect(base_url()."ApproveKegiatanMahasiswa");
      }else{

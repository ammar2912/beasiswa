  ...
  <div class="col-12">
    <div class="card card-cascade narrower z-depth-1">
      <div class="view view-cascade gradient-card-header blue-gradient narrower py-2 mx-4 mb-3 d-flex justify-content-between align-items-center">
        ...
        <a href="" class="white-text mx-3">Daftar Data Prestasi Ormawa</a>
        <div>
    ...
    <div class="card-body">
      <form method="post" action="<?=base_url('DataPrestasiOrmawa/export_data')?>">
      <div class="row mb-4">
        <div class="col-3">
          <label for="opsi_jurusan">Jenis Ormawa</label>
          <select name="opsi_jurusan" id="jurusan" class="form-control mt-1" required="1">
            <option value="">Pilih Jenis Ormawa</option>
            <option value="all">Semua Jenis Ormawa</option>
            <?php
            foreach ($jenisormawa as $value) {
              ?>
              <option value="<?=$value->id;?>">Ormawa <?=$value->jenis;?></option>
              <?php
            }
            ?>
          </select>
        ...
        <div class="col-3">
          <label for="opsi_prodi">Ormawa</label>
          <select name="opsi_prodi" id="prodi" class="form-control mt-1" required="1" >
          </select>
        </div>
        <div class="col-3">
          <br>
      ...
      <div class="table-responsive">
        ...
        <tbody>
          <?php $no=1; foreach ($dataprestasiormawa as $data) {?>
            <?php
            ...
            <td>
              ...
              <td>
                <a href="<?php base_url()?>DataPrestasiOrmawa/data_foto/<?php echo base64_encode($id_check);?>">
                  <b>
              ...
              <td>
                <a href="<?php base_url()?>DataPrestasiOrmawa/edit/<?php echo base64_encode($id_check);?>">
...
$(document).ready(function(){
  ...
  $('#jurusan').change(function(){
    ...
    var fail = null;
    if(id == 'all'){
      html = '<option value='+all+'>Semua Ormawa</option>';
      $('#prodi').html(html);
      $('#value').val('all');
    }else{
      $.ajax({
        url : "<?php echo base_url('DataPrestasiOrmawa/get_ormawa');?>",
        method : "POST",
        data : {id: id},
        async : true,
        dataType : 'json',
        success: function(data){
          if(data != 'null'){
            html = '<option value=>Pilih Ormawa</option>';
            for(i=0; i<data.length; i++){
              html += '<option value='+data[i].id+'>'+data[i].nama_ormawa+'</option>';
            }
          }else{
            html = '<option value='+fail+'>Belum Ada Ormawa</option>';
          }
          $('#prodi').html(html);
        }
      });

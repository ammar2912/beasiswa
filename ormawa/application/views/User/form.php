    ...
    <div class="form-group animated flipIn">
      ...
      <select class="form-control myselect2" name="nopeg" required>
        ...
        <?php foreach ($pegawai as $value): ?>
          <option value="<?php echo $value->idpegawai?>"><?php echo $value->nama;?></option>
        <?php endforeach; ?>
      </select>
    </div>
  </div>
          ...
          <?php foreach ($kat['menu'] as $menu): ?>
            <?php if (empty($menu['sub'])): ?>
              <optgroup label="<?php echo strtolower($kat['nama'])?>">
                <option value="<?php echo $menu['id']?>"><?php echo strtolower($menu['nama'])?></option>
              </optgroup>
              <?php else: ?>
              <?php foreach ($menu['sub'] as $sub): ?>
                <optgroup label="<?php echo strtolower($menu['nama'])?>">
                  <option value="<?php echo $sub['id']?>"><?php echo strtolower($sub['nama'])?></option>
                </optgroup>
